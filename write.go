	...
	go func() {
		...
		for {
			...
			case writeBlockReq := <-writeBlockChan:
				...
				printToLog(fmt.Sprintf("%d verified | %d failed | %d orphaned", len(verified), len(failed), len(orphans)))
				orphaned = orphans
				...
				if writeBlockReq.Type == "sync" {
					for i := range AllPeers {
						exclusion = append(exclusion, AllPeers[i])
					}
				}
				if writeBlockReq.Type == "received" || writeBlockReq.Type == "mined" {
					exclusion = []*Peer{writeBlockReq.From}
				}

				for i := range len(verified) {
					go broadcastBlock(verified[i], exclusion)
				}
				...
				if miningState.Active {
					...
					newHeightsChan <- heights // Notify miners of new blocks
				}
...
// readBlocks returns the blocks requested by height from file
func readBlocks(datFile, directoryFile, offsetFile *os.File, heights []int) ([][]Block, error) {
...
// getChainStats gets the latest height and total blocks from file
